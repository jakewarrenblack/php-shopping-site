...
class Validator
{
    ...
    private function validate_rule($key, $rule_str)
    {
        ...
        switch ($rule_name) {
            case "present":
                $valid = $this->validate_rule_present($key, $options);
                break;
            case "minlength":
                $valid = $this->validate_rule_minlength($key, $options);
                break;
            case "maxlength":
                $valid = $this->validate_rule_maxlength($key, $options);
                break;
            case "email":
                $valid = $this->validate_rule_email($key, $options);
                break;
            case "float":
                $valid = $this->validate_rule_float($key, $options);
                break;
            case "integer":
                $valid = $this->validate_rule_integer($key, $options);
                break;
            case "min":
                $valid = $this->validate_rule_min($key, $options);
                break;
            case "max":
                $valid = $this->validate_rule_max($key, $options);
                break;
            case "boolean":
                $valid = $this->validate_rule_boolean($key, $options);
                break;
            case "match":
                $valid = $this->validate_rule_match($key, $options);
                break;
            case "in":
                $valid = $this->validate_rule_in($key, $options);
                break;
            case "not_in":
                $valid = $this->validate_rule_not_in($key, $options);
                break;
            case "subset":
                $valid = $this->validate_rule_subset($key, $options);
                break;
            case "not_subset":
                $valid = $this->validate_rule_not_subset($key, $options);
                break;
            default:
                throw new Exception("Error in validation rule for " . $key);
                break;
